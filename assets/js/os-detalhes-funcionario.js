document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Função para verificar se o usuário logado é responsável pela OS
    const isUserResponsible = (osUsuarios) => {
        try {
            ...
            console.log('Usuários responsáveis pela OS:', osUsuarios);
            
            if (!osUsuarios || osUsuarios.length === 0) {
                console.log('Nenhum responsável na OS');
                return false;
            }
            
            const isResponsible = osUsuarios.some(u => {
                console.log(`Comparando: ${u.id} === ${userId}`, u.id == userId);
                return u.id == userId;
            });
            
            console.log('Usuário é responsável?', isResponsible);
    ...
    const updateStatus = async (newStatus) => {
        try {
            ...
            saveStatusBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i> Salvando...';
            ...
            statusEl.classList.add(
                newStatus === 'Concluida' ? 'bg-success' : 
                newStatus === 'Em andamento' ? 'bg-warning' : 
                newStatus === 'Cancelada' ? 'bg-danger' : 
                'bg-secondary'
            );
            ...
            `;
            editStatusCard.insertAdjacentElement('afterend', successAlert);
            ...
            errorAlert.innerHTML = `
                ...
                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
            `;
            editStatusCard.insertAdjacentElement('afterend', errorAlert);
            ...
            saveStatusBtn.innerHTML = '<i class="fas fa-save me-1"></i> Salvar Status';
        }
    };
    ...
    try {
        ...
        // TEMPORÁRIO: Mostrar sempre o card de edição para testes
        console.log('Mostrando card de edição de status (modo teste - sempre visível)');
        editStatusCard.classList.remove('d-none');
        
        // Verificação comentada temporariamente
        // const userIsResponsible = isUserResponsible(os.usuarios);
